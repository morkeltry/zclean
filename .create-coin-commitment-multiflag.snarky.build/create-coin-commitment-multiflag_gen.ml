...
open Extended_lib
let main (mh0 : Field.t) (nf : Field.t) (cm_new : Field.t) () =
  ...
  let hash_commitment (secret : Field.t) (val1 : Field.t) (val2 : Field.t) =
    ...
    let hashable = secret_bool in hash_bits hashable in
  ...
  let (r, v, flag, tree_threeple, r', flag', v') =
    exists
      ...
      ~compute:(fun () ->
                  let r::v::flag::l1_sib_string::l2_sib_string::l3_sib_string::r'::v'::flag'::[]
  ...
  assert_r1cs t' (Field.constant (Field.Constant.of_string "1")) cm_new
module Toplevel_param_module__ =
  struct
    type result = unit
    type computation = Field.t -> Field.t -> Field.t -> unit -> unit
    type public_input =
      Field.Constant.t -> Field.Constant.t -> Field.Constant.t -> unit
    let compute = main
    let public_input =
      let open Data_spec in [Field.typ; Field.typ; Field.typ]
    let read_input a =
      match a with
      | f_2::f_1::f_0::[] ->
          let open H_list in
            [Field.Constant.of_string f_2;
            Field.Constant.of_string f_1;
            Field.Constant.of_string f_0]
      | _ -> failwith "Wrong number of arguments: expected 3"
  end
